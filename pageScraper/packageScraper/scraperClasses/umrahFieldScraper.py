import re
import json
from .baseFieldScraper import BaseFieldScraper
from pageScraper.regexHelpers import hasKeywordPattern, regexSearch
from pageScraper.packageScraper.packageRegexConsts import ISLAMIC_MONTH_RE
from utils import getProjectRoot
from pageScraper.packageScraper.dateFieldHelpers import fillMissingDateFields, normalizeIslamicMonth
from pageScraper.schema.fields import UmrahField, BaseField

class Umrah_FieldScraper(BaseFieldScraper):
  SCHEMA = json.loads((getProjectRoot() / "pageScraper" / "schema" / "umrahPackage.json").read_text())

  @classmethod
  def get_scrapers(cls):
    return {**super().get_scrapers(), UmrahField.IS_ZIYARAT_INCLUDED: cls.scrapeIsZiyaratIncluded, UmrahField.SEASON: cls.scrapeSeason,
            UmrahField.MONTH: cls.scrapeMonth, UmrahField.ISLAMIC_MONTH: cls.scrapeIslamicMonth}

  @classmethod
  def run(cls, soup):
    scrapedInfo = super().run(soup)
    inferedDateFields = fillMissingDateFields(scrapedInfo.get(BaseField.YEAR), scrapedInfo.get(UmrahField.SEASON),
                                scrapedInfo.get(UmrahField.MONTH), scrapedInfo.get(UmrahField.ISLAMIC_MONTH))
    scrapedInfo.update(inferedDateFields)
    return scrapedInfo

  @staticmethod
  def scrapeIsZiyaratIncluded(textList):
    ziyaratPattern = r"\bziy?ara[th]s?\b"
    result = hasKeywordPattern(ziyaratPattern, textList)
    return False if result is None else result

  @staticmethod
  def scrapeSeason(textList):
    seasonRegex = re.compile(r"\b(winter|spring|summer|autumn|fall)\b", re.IGNORECASE)
    match = regexSearch(seasonRegex, textList)
    if not match:
      return None

    season = match.group(1).lower()
    if season == "fall":
      season = "autumn"

    return season.capitalize()

  @staticmethod
  def scrapeMonth(textList):
    monthRegex_withoutMay = re.compile(r"\b(january|february|march|april|june|july|august|september|october|november|december)\b", re.IGNORECASE)
    match = regexSearch(monthRegex_withoutMay, textList)
    if match:
      return match.group(1).capitalize()
    else:
      match = regexSearch(re.compile(r"\bMay\b"), textList) #NOTE: separating May with captilisation required may not be enough
                                                                #e.g. 'PRICES MAY VARY' this would match the month as May
      if match:
        return "May"
      else:
        return None

  @staticmethod
  def scrapeIslamicMonth(textList):
    match = regexSearch(ISLAMIC_MONTH_RE, textList)
    if not match:
      return None

    return normalizeIslamicMonth(match.group(1))