import re
from jsonschema import validate, ValidationError
from pageScraper.regexHelpers import hasKeywordPattern, regexSearch
from pageScraper.packageScraper.packageRegexConsts import DEPARTURE_CITY_RE
from pageScraper.commonRegex import TOTAL_DAYS_REGEX 
from hijridate import Hijri
from datetime import date
from pageScraper.schema.fields import BaseField

class BaseFieldScraper:
  SCHEMA = None
  PPP_MINMAX = [None, None]
  TOTALDAYS_MINMAX = [None, None]
  YEAR_MIN = None

  @classmethod
  def __init_subclass__(cls, **kwargs):
    super().__init_subclass__(**kwargs)
    if cls.SCHEMA is not None:
        cls._load_bounds()
        cls.removeNonPacakgeFields()

  @classmethod 
  def removeNonPacakgeFields(cls):
      REMOVE = frozenset([BaseField.COMPANY, BaseField.URL, BaseField.MAKKAH_HOTEL, BaseField.MADINAH_HOTEL])
      cls.SCHEMA = {**cls.SCHEMA,
      "properties": {k: v for k, v in cls.SCHEMA["properties"].items() if k not in REMOVE},
      "required": [r for r in cls.SCHEMA["required"] if r not in REMOVE],
      }
  
  @classmethod
  def _load_bounds(cls):
    properties = cls.SCHEMA["properties"]
    cls.PPP_MINMAX = [properties[BaseField.PPP]["minimum"], properties[BaseField.PPP]["maximum"]]
    cls.TOTALDAYS_MINMAX = [properties[BaseField.TOTAL_DAYS]["minimum"], properties[BaseField.TOTAL_DAYS]["maximum"]]
    cls.YEAR_MIN = properties[BaseField.YEAR]["minimum"]

  @classmethod
  def get_scrapers(cls):
    scrapers = {BaseField.PPP: cls.scrapePPP, BaseField.YEAR: cls.scrapeYear, BaseField.TOTAL_DAYS: cls.scrapeTotalDays, 
                BaseField.TIER: cls.scrapeTier, BaseField.STARS: cls.scrapeStars, BaseField.DEPARTURE_CITY: cls.scrapeDepartureCity,
                BaseField.IS_VISA_INCLUDED: cls.scrapeIsVisaIncluded}
    return scrapers

  @classmethod
  def run(cls, soup, validate=False):
    scrapedInfo = {}
    textList = list(soup.stripped_strings)
    for field, fn in cls.get_scrapers().items():
      if field == BaseField.TOTAL_DAYS:
        scrapedInfo[field] = fn(soup)
      else:
        scrapedInfo[field] = fn(textList)

    if validate:
      error = validate(scrapedInfo)
      if error:
        #add in logging later
        return None

    return scrapedInfo

  @classmethod
  def validate(cls, scrapedInfo: dict):
    try:
      validate(instance=scrapedInfo, schema=cls.SCHEMA)
    except ValidationError as e:
      return e.message

  @classmethod
  def scrapeTotalDays(cls, soup):
    totalDays = -1
    HEADING_TAGS = ["h1", "h2", "h3", "h4", "h5", "h6", "small", "strong"]
    for tag in soup.find_all(HEADING_TAGS):
      match = regexSearch(TOTAL_DAYS_REGEX, list(tag.stripped_strings))
      if match:
        totalDays = int(match.group(1))
        break
    
    if totalDays >= cls.TOTALDAYS_MINMAX[0] and totalDays <= cls.TOTALDAYS_MINMAX[1]:
      return totalDays
    else:
      return None

  @classmethod
  def scrapePPP(cls, textList): 
    FX = {"GBP": 1.0, "USD": 0.75, "EUR": 0.86, "SAR": 0.20}
    SYMBOLS = {
        "£": "GBP", "gbp": "GBP",
        "€": "EUR", "eur": "EUR",
        "us$": "USD", "$": "USD", "usd": "USD",
        "sar": "SAR",
    }
    _s = "|".join(re.escape(k) for k in SYMBOLS)
    _n = r"\d[\d,]*(?:\.\d{1,2})?"
    PRICE_REGEX = re.compile(rf"(?:({_s})\s*({_n})|({_n})\s*({_s}))", re.IGNORECASE)

    blob = " ".join(textList)
    match = PRICE_REGEX.search(blob)
    if not match:
      return None
    else:
      symbol = (match.group(1) or match.group(4)).lower()
      number = (match.group(2) or match.group(3)).replace(",", "")
      cur = SYMBOLS.get(symbol)

      if not cur or cur not in FX:
        return None
      
      gbp = round(float(number) * FX[cur])
      if gbp >= cls.PPP_MINMAX[0] and gbp <= cls.PPP_MINMAX[1]:
        return gbp
      else:
        return None
    
  @classmethod
  def scrapeYear(cls, textList):
    yearRegex = re.compile(r"\b(20\d{2}|14\d{2})\b", re.IGNORECASE)
    match = regexSearch(yearRegex, textList)
    if not match:
      return None
    
    max_year = date.today().year + 1
    year = int(match.group(1))

    if year >= 1400 and year < 1500:
      try:
        year = Hijri(year, 1, 1).to_gregorian().year
      except Exception:
        return None
    
    if year >= cls.YEAR_MIN and year <= max_year:
      return year
    else:
      return None 

  @staticmethod
  def scrapeTier(textList):
    tierRegex = re.compile(r"\b(luxury|premium|economy)\b", re.IGNORECASE)
    match = regexSearch(tierRegex, textList)
    if match:
      return match.group(1).lower()
    else: 
      return None

  @staticmethod
  def scrapeStars(textList):
    starsRegex = re.compile(r'\b([1-5])\s*(?:-?\s*star|stars?)\b', re.IGNORECASE)
    match = regexSearch(starsRegex, textList)
    if match:
      return int(match.group(1))
    else:
      return None

  @staticmethod
  def scrapeDepartureCity(textList):
    match = regexSearch(DEPARTURE_CITY_RE, textList)
    if not match:
      return None

    return match.lastgroup 

  @staticmethod
  def scrapeIsVisaIncluded(textList):
    visaPattern = r"\bvisas?\b"
    result = hasKeywordPattern(visaPattern, textList)
    return False if result is None else result
