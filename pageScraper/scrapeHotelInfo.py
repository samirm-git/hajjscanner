...
from scraperModels import HotelScraper
...
def checkCityinText(text, city: City):
  if city.regex.search(text):
    return True
  else:
    return False
...
#TODO: COMBINE MAKKAH AND MADINAH INTO scrapeHotelInfo RATHER THAN REQUIRING EXTRA CALLS
#AS SCRAPING MAKKAH/MADINAH STILL REQUIRES THE OTHER AS SEEN IN "otherCity"
def findCityContainers(soup, city: City):
  """Walk the soup once and collect all containers that mention `city`
  but not the other city."""

  otherCity = City.MADINAH if city == City.MAKKAH else City.MAKKAH
...
def scrapeHotelInfo(soup, url, city: City):
  matchedContainers = findCityContainers(soup, city)
  if not matchedContainers:
      return None

  mergedSoup = _mergeContainers(matchedContainers)
  hotelScraper = HotelScraper(soup=mergedSoup, url=url, city=city)
  hotelInfo = hotelScraper.run()
